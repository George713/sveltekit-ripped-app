...
export const eatenKcal = derived([foodLibrary, plannedItems, estimatesLog], ([$foodLibrary, $plannedItems, $estimatesLog]) => {
    const kcalFromPlannedItems = $plannedItems.reduce((sum, item) => sum + foodLibrary.getKcalByIndex(item.foodId) * (item.eaten ? 1 : 0), 0)
...
export const eatenProtein = derived([foodLibrary, plannedItems, estimatesLog], ([$foodLibrary, $plannedItems, $estimatesLog]) => {
    const proteinFromPlannedItems = $plannedItems.reduce((sum, item) => sum + foodLibrary.getProteinByIndex(item.foodId) * (item.eaten ? 1 : 0), 0)
